    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        logger.audit({
          ...
          newValues,
          metadata,
          username: context?.logContext?.username,
          userId: context?.logContext?.userId,
          ip: context?.logContext?.ip,
          userAgent: context?.logContext?.userAgent,
          timestamp: new Date().toISOString(),
          requestId: context?.logContext?.requestId,
        })
...
function extractValueFromPath(path: string, context: { args: any[]; result: any }): any {
  ...
  const root =
    parts[0] === 'args' ? context.args : parts[0] === 'result' ? context.result : undefined
