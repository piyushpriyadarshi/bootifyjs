...
### Fixed
- **CLI:** `npx bootifyjs new <name> --yes` (or `--skip-install`) without an
  explicit `--template` now defaults to `minimal` instead of failing with
  `Unknown template ''`.
- **Packaging:** `dist/cli/bin.js` is marked executable during build
  (`postbuild`), so the published `bin` entry has the right permission in
  every installer.
